...
# ==========================================
# 0. STREAMLIT PAGE CONFIG & HEADER
# ==========================================
st.set_page_config(
    page_title="Project Alpha-NSE | Production SMC & ML Engine",
    page_icon="🏛️",
    layout="wide"
)

st.title("🏛️ Project Alpha-NSE: Institutional Filing & SMC Scanner")
st.markdown("*SEBI LODR Corporate Filings, FII/DII Order Flow, Session VWAP, & Dynamic SMC Lifecycle Engine*")

MODEL_FILE = "colab_ai_model.pkl"
SCALER_FILE = "colab_scaler.pkl"
EXACT_FEATURES = [
...
# ==========================================
# 1. ONE-TIME MODEL PERSISTENCE & LOADING
# ==========================================
def train_and_save_fallback_model():
    """Generates an initial ML model and saves it locally if .pkl files are missing."""
    st.info("⚡ Generating initial offline ML model artifacts (One-Time Execution)...")
    np.random.seed(42)
    N = 1000
    X_dummy = pd.DataFrame({
        'RVOL': np.random.uniform(0.5, 5.0, N),
        'ATR_Pct': np.random.uniform(0.2, 3.0, N),
        'RSI': np.random.uniform(20, 80, N),
        'Liquidity_Sweep_High': np.random.choice([0, 1], N),
        'Liquidity_Sweep_Low': np.random.choice([0, 1], N),
        'Bullish_FVG': np.random.choice([0, 1], N),
        'Bullish_OB': np.random.choice([0, 1], N),
        'Pattern_Flag_Breakout': np.random.choice([0, 1], N),
        'Market_Sentiment': np.random.uniform(-2.0, 2.0, N)
    })
    
    # Synthetic target based on SMC confluence
    y_dummy = (
        (X_dummy['RVOL'] > 1.8) & 
        (X_dummy['Bullish_OB'] == 1) & 
        (X_dummy['RSI'] < 65) & 
        (X_dummy['Market_Sentiment'] > -0.5)
    ).astype(int)
...
# Sidebar Setup & Model Controls
st.sidebar.success(f"✅ AI Engine Configured: {len(EXACT_FEATURES)} Features Persistent")

if st.sidebar.button("🔄 Re-Train ML Model (Offline Dataset)"):
...
active_sectors = st.sidebar.multiselect(
    ...
    default=["Banking", "IT", "Auto", "Energy"],
    help="Sectors to monitor for confirmed corporate filings."
)

# ==========================================
# 2. MARKET SENTIMENT & FII/DII FLOW PROXY
# ==========================================
SECTOR_MAP = {
...
# Render Top Index Dashboard Bar
col1, col2, col3, col4 = st.columns(4)
col1.metric("Nifty 50 Trend", idx_trends.get("^NSEI", "Active"))
col2.metric("Nifty Midcap", idx_trends.get("^NSEMDCP50", "Active"))
col3.metric("Nifty Smallcap", idx_trends.get("Smallcap", "Active"))
col4.metric("Net FII/DII Order Flow", f"₹{inst_flow['Net_Flow'] / 100:.2f} Cr", inst_flow["Sentiment"], delta_color="normal" if inst_flow["Net_Flow"] > 0 else "inverse")
...
# ==========================================
# 4. STRICT SEBI LODR & NEWS SCRAPER
# ==========================================
def evaluate_official_filings(ticker, df_5m):
    """Enforces strict timestamp age validation to eliminate outdated news."""
    try:
        meta = STOCK_METADATA.get(ticker, {})
        query_str = meta.get("name_query", ticker) + " corporate filing NSE BSE announcement"
        ...
        if not items: return 0.0, "No official filing"
        ...
        name_keyword = meta.get("name_query", clean_ticker)
        ...
        if age_hours > 48.0:
            return 0.0, f"Archived Filing ({int(age_hours)}h old)"

        tier_1_bullish = ["financial results", "net profit up", "order win", "secures contract", "bonus issue", "buyback", "board meeting outcome", "dividend declared", "acquisition", "profit"]
        tier_1_bearish = ["sebi penalty", "investigation", "default", "fraud", "resignation", "net profit down", "cbi raid", "adverse ruling"]

        sentiment = 0
        if any(w in title_lower for w in tier_1_bullish): sentiment = 1
        elif any(w in title_lower for w in tier_1_bearish): sentiment = -1

        if sentiment == 0: return 0.0, f"Routine Filing: {title_text[:30]}..."

        base_score = 20.0 * sentiment if age_hours <= 12 else 10.0 * sentiment
        status = "🏛️ BREAKING LODR" if age_hours <= 4 else "🌙 OVERNIGHT FILING"
        ...
        if len(df_5m) > 20 and age_hours > 0.5:
            ...
            if sentiment == 1 and recent_return > 3.0:
                return -15.0, f"🛑 GAP TRAP (+{recent_return:.1f}% Priced In): {title_text[:25]}..."
            if sentiment == -1 and recent_return < -3.0:
                return 15.0, f"🛑 EXHAUSTED DUMP ({recent_return:.1f}% Priced In): {title_text[:25]}..."
...
# ==========================================
# 5. TECHNICAL & ADVANCED SMC ENGINE
# ==========================================
...
def compute_session_vwap(df_5m):
    df = df_5m.copy()
    df['Date'] = df.index.date
    df['TP'] = (df['High'] + df['Low'] + df['Close']) / 3
    df['VP'] = df['TP'] * df['Volume']
    vwap = df.groupby('Date')['VP'].cumsum() / (df.groupby('Date')['Volume'].cumsum() + 1e-5)
    return vwap

def detect_chart_patterns(df_5m):
    patterns = []
    if len(df_5m) < 30: return patterns
    close = df_5m['Close']
    high = df_5m['High']
    low = df_5m['Low']
    ...
    range_old = high.iloc[-30:-15].max() - low.iloc[-30:-15].min()
    range_new = high.iloc[-15:].max() - low.iloc[-15:].min()
    if range_new < range_old * 0.5:
        patterns.append("Triangle Compression")
        
    return patterns

def track_smc_zones(df_5m, lookback=60):
    """
    AXIS-BOUND DYNAMIC SMC ENGINE:
    Identifies FVG, origin Order Block candle index (start_idx), 
    and checks forward candles for entries or total mitigation.
    """
    ...
    for i in range(len(df_5m) - actual_lookback, len(df_5m) - 1):
        # 1. Bullish Order Block (Red candle before green displacement with FVG)
        if df_5m['Close'].iloc[i] < df_5m['Open'].iloc[i] and df_5m['Close'].iloc[i+1] > df_5m['Open'].iloc[i+1]:
            ...
            # Forward Mitigation Validation
            forward_closes = df_5m['Close'].iloc[i+1:]
            if not (forward_closes < ob_bottom).any(): # Has not been completely broken
                max_fwd = df_5m['High'].iloc[i+1:].max()
                dist_pct = ((last_price - ob_top) / ob_top) * 100
                
                if max_fwd > ob_top + (2.0 * atr): state, state_val = "🔴 MITIGATED", -1
                elif ob_bottom <= last_price <= ob_top or ob_bottom <= last_low <= ob_top: state, state_val = "🟢 ENTRY READY", 2
                elif 0 < dist_pct <= 0.5: state, state_val = "🟡 PULLBACK NEAR", 1
                else: state, state_val = "⏸️ ZONE CREATED", 0
                
                if state_val >= 0:
                    active_zones.append({
                        'type': 'Bullish OB', 'top': ob_top, 'bottom': ob_bottom, 
                        'start_idx': i, 'start_time': df_5m.index[i],
                        'age': len(df_5m)-1-i, 'state': state, 'state_val': state_val
                    })
        ...
        if df_5m['Close'].iloc[i] > df_5m['Open'].iloc[i] and df_5m['Close'].iloc[i+1] < df_5m['Open'].iloc[i+1]:
            ...
            forward_closes = df_5m['Close'].iloc[i+1:]
            if not (forward_closes > ob_top).any():
                min_fwd = df_5m['Low'].iloc[i+1:].min()
                dist_pct = ((ob_bottom - last_price) / ob_bottom) * 100
                
                if min_fwd < ob_bottom - (2.0 * atr): state, state_val = "🔴 MITIGATED", -1
        ...
        if i > 15:
            ...
            if df_5m['Low'].iloc[i] < recent_min and df_5m['Close'].iloc[i] > recent_min:
                ...
                if not (df_5m['Close'].iloc[i+1:] < sweep_lvl).any():
                    if 0 <= ((last_price - sweep_lvl) / sweep_lvl) * 100 <= 0.3: state, state_val = "🟢 ENTRY READY", 2
                    else: state, state_val = "⏸️ ZONE CREATED", 0
                    active_zones.append({
            ...
            if df_5m['High'].iloc[i] > recent_max and df_5m['Close'].iloc[i] < recent_max:
                ...
                if not (df_5m['Close'].iloc[i+1:] > sweep_lvl).any():
                    if 0 <= ((sweep_lvl - last_price) / sweep_lvl) * 100 <= 0.3: state, state_val = "🔴 SHORT READY", 2
                    else: state, state_val = "⏸️ ZONE CREATED", 0
                    active_zones.append({
                        'type': 'Sweep High', 'top': sweep_lvl, 'bottom': sweep_lvl * 0.999, 
                        'start_idx': i, 'start_time': df_5m.index[i],
                        'age': len(df_5m)-1-i, 'state': state, 'state_val': state_val
                    })
...
if lock_signals and st.session_state.locked_results is not None:
    ...
    with st.spinner("Executing ML Inference, Parsing LODR Filings, & Mapping SMC Structures..."):
        results = []
        ...
        for ticker in selected_tickers:
            try:
                ...
                day_open = float(df_1d["Open"].dropna().iloc[-1])
                ...
                # Pinbar Trigger
                trigger = ""
                for _, row_b in df_5m.tail(3).iterrows():
                    body = abs(row_b['Open'] - row_b['Close'])
                    wick_up = row_b['High'] - max(row_b['Open'], row_b['Close'])
                    wick_down = min(row_b['Open'], row_b['Close']) - row_b['Low']
                ...
                active_zones = track_smc_zones(df_5m, lookback=60)
                ...
                if active_zones:
                    ...
                    zone_context = f"Age: {best_zone['age']} bars | VWAP: ₹{last_vwap:.1f} | " + filing_context
                ...
                # ML Feature Vector Transformation
                X_df = pd.DataFrame([{
                    'RVOL': rvol, 'ATR_Pct': atr_pct, 'RSI': rsi_val,
                    'Liquidity_Sweep_High': sweep_high_feat, 'Liquidity_Sweep_Low': sweep_low_feat,
                    'Bullish_FVG': 1 if bull_ob_feat else 0, 'Bullish_OB': bull_ob_feat,
                    'Pattern_Flag_Breakout': 1 if "Bull Flag Breakout" in chart_patterns else 0, 
                    'Market_Sentiment': market_sentiment
                }])
                
                X_scaled = scaler.transform(X_df) if scaler else X_df
                prob = float(model.predict_proba(X_scaled)[0][1]) if hasattr(model, "predict_proba") else 0.5
                
                # Dynamic Structural SL & TP Calculation
                major_bsl = float(high_5m.tail(150).max())  
                major_ssl = float(low_5m.tail(150).min())  

                if best_zone and best_zone['state_val'] >= 1:
                    is_long = "Bullish" in best_zone['type'] or "Low" in best_zone['type']
                    if is_long:
                        sl_price = best_zone['bottom'] - (0.15 * atr_14_val)
                        tgt_price = major_bsl if major_bsl > last_price + (1.5 * atr_14_val) else last_price + (3.0 * atr_14_val)
                    else: 
                        sl_price = best_zone['top'] + (0.15 * atr_14_val)
...
# ==========================================
# 7. DASHBOARD DISPLAY & VISUAL CHARTS
# ==========================================
if "locked_results" in st.session_state and st.session_state.locked_results is not None:
    ...
    if df_5m_chart is not None and not df_5m_chart.empty:
        ...
        # Overlay Session VWAP
        vwap_vals = compute_session_vwap(df_5m_chart)
        ...
        fig.add_trace(go.Scatter(
            x=df_5m_chart.index, y=vwap_vals, mode='lines', 
            line=dict(color='orange', width=1.5), name="Session VWAP"
        ))
        
        # Render Precise Axis-Bound Order Blocks (Time-bounded from Creation Candle)
        top_zones = track_smc_zones(df_5m_chart, lookback=60)
        if top_zones:
            for z in top_zones[:2]: # Show top active unmitigated zones
                color = "rgba(0, 255, 0, 0.25)" if "Bullish" in z['type'] or "Low" in z['type'] else "rgba(255, 0, 0, 0.25)"
                border_color = "green" if "Bullish" in z['type'] or "Low" in z['type'] else "red"
                
                # Rectangular shape pinned strictly from Creation Time (start_time) to latest candle
                fig.add_shape(
                    type="rect",
                    x0=z['start_time'], y0=z['bottom'],
                    x1=df_5m_chart.index[-1], y1=z['top'],
                    fillcolor=color,
                    line=dict(color=border_color, width=1),
                )
            
        fig.update_layout(
            title=f"{top_ticker} - Dynamic SMC Zone & VWAP Alignment", 
